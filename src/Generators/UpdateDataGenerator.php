...
class UpdateDataGenerator
{
    ...
    {
        return 'Update'.$this->modelNameInferrer->infer($schema->name).'Data';
    }

    protected function loadStub(): string
    {
        $custom = base_path('stubs/vendor/api-from-table/data.update.stub');
        if ($this->files->exists($custom)) {
            return $this->files->get($custom);
        }

        $package = __DIR__.'/../../stubs/data.update.stub';
        if (! $this->files->exists($package)) {
            throw new RuntimeException("Update data stub not found at [{$package}].");
        }
